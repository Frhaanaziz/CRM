...
export default defineEventHandler(async (event) => {
    ...
    const zodResult = leadSchema.pick({ id: true }).safeParse(JSON.parse(await readBody(event)));
    ...
    const { error } = await supabase.from('Leads').update({ status: 'qualified' }).eq('id', id);
    if (error) {
        console.error('Error qualifying lead (SERVER)', error);
